...
class TwoCompNeuron:
    def __init__(self, key: jnp.ndarray, n_neurons: int, n_inputs: int, config: NeuronConfig):
        ...
        # Per-neuron adaptation parameters (trainable). Warm-started at the fixed
        # config values, so training starts identical to the fixed-value baseline.
    ...
    def forward_step(carry, dend_in, soma_in, t, alpha_s, alpha_d, T_p, config, alpha_w,
        ...
        v = v_pre_reset * (1 - o)
        w = alpha_w * w_prev + (1 - alpha_w) * a_adapt * v_pre_reset + b_adapt * o
