...
#    grad W_d^(l) = sum_t (do^(l)[t]·γ·s'_d^(l)[t]) ⊗ P̂^(l)[t]
...
def _forward_backward(
    ...
    def _layer_forward(h_state, dend_in_l, soma_in_l, presyn, t, tp_l, a_l, b_l):
        ...
        # gradient; the advanced trace is stored for the next step.
        elig_a_new = alpha_w * elig_a_prev + (1 - alpha_w) * v_pre_l
    ...
    def step2(carry, inputs):
        ...
        for l in reversed(range(L)):
            ...
            new_grad_dend[l] = new_grad_dend[l] + (
                (delta_o * config.gamma * hps[l])[:, None] * Phat[l]
            )
...
class Network:
    ...
    def _update_weights(self, g_dend_list, g_soma_list, g_readout,
        ...
        # Scale the adaptation-parameter grads (effective smaller LR for a/b).
        if self.adapt_lr_scale != 1.0:
            g_a_list = [g * self.adapt_lr_scale for g in g_a_list]
            g_b_list = [g * self.adapt_lr_scale for g in g_b_list]
    ...
    def adapt_stats(self):
        """Per-layer (mean, std) of the trained a_adapt/b_adapt params (diagnostic)."""
        stats = {}
