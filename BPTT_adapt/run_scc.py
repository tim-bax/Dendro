...
def parse_args():
    ...
    p.add_argument("--a_adapt", type=float, default=0.0,
                   help="Subthreshold adaptation WARM-START value (per-neuron a_adapt is "
                        "trained; this is its init, default 0.0 = start from disabled).")
    p.add_argument("--b_adapt", type=float, default=0.0,
                   help="Spike-triggered adaptation WARM-START value (per-neuron b_adapt is "
                        "trained; this is its init, default 0.0 = start from disabled).")
    p.add_argument("--adapt_lr_scale", type=float, default=1.0,
                   help="Multiplier on the a_adapt/b_adapt gradients (effective smaller LR "
                        "for the adaptation params). Try 0.1-0.3 if adaptation destabilizes.")
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if gnorm_count > 0:
            ...
            adapt_str = "  ".join(
                f"{k}={m:+.3f}±{s:.3f}" for k, (m, s) in adapt.items()
            )
            print(f"         adapt (mean±std): {adapt_str}", flush=True)
