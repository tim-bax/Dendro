...
# Experiment: decay the DENDRITIC eligibility trace with its own time constant
# (ms), independent of the membrane's tau_dend. Set equal to the tau_dend you
# run with to recover the exact e-prop trace (dmu_dw == d mu / d w).
_TAU_ELIG_DEND_MS = 15.0
...
def _forward_backward(
    ...
    Why two passes instead of the old single-pass (J,N,K) accumulator: the LI
    readout is linear, so its per-output surrogate is identically 1. That makes
    the time-summed gradient core independent of the output index j, so we can
    contract the error e first — readout_seed = w_readout^T e, a length-N learning
    signal — and accumulate only (N,K) gradients. Pass 1 runs the forward (with
    dropout) to get the readout voltage and hence e; pass 2 re-runs the forward
    and accumulates the hidden grads. This is a pure linear reassociation of the
    old rule, i.e. the same gradient up to floating-point summation order.

    Pass 2 needs no dropout: the hidden layer reads the RAW input x, and none of
    its eligibilities/surrogates depend on the mask (dropout only rescales the
    hidden spikes the readout sees, which pass 2 never touches).
    """
    ...
    def _hidden_zeros():
        # The 9-slot hidden carry (see TwoCompNeuron.init_carry), built inline so
        # this pure function needs no per-sample carry args under vmap.
        return (
            ...
            jnp.zeros(n_inputs),                   # E_soma
            jnp.zeros((n_hidden, n_inputs)),       # dmu_dw
            jnp.zeros((n_hidden, n_inputs)),       # dmu_dw_at_tprime
    ...
    grad_readout = (e[:, None] * sum_Er[None, :]) / T
    readout_seed = w_readout.T @ e                 # (N,) error projected to hidden

    # rho[t] = B[t]/T with B[t] = Σ_{s>=t} alpha_m^{s-t}, the readout kernel's
    # forward spread of a hidden spike into the time-averaged logit. Applying
    # rho[t] per step equals the old F_d running filter Σ_t B[t]·g[t] / T.
    rho = (1.0 / T) * (1.0 - alpha_m ** (T - time_indices)) / (1.0 - alpha_m)

    # ── Pass 2: re-run forward (no dropout) → accumulate (N,K) hidden grads ─
    def step2(carry, inputs):
        h_carry, grad_soma, grad_dend = carry
        dend_in, soma_in, x_t, t, rho_t = inputs
        ...
        # Decouple the eligibility-trace decay from the membrane's alpha_d so the
        # trace can carry a longer memory than tau_dend (constant-folded by JIT).
        a_elig = jnp.exp(-config.dt / _TAU_ELIG_DEND_MS) #
        dmu_new, dmu_atp_new = TwoCompNeuron.update_dendritic_eligibility(
            dmu_c, dmu_atp_c, x_t.astype(jnp.float64), h_h_prev, a_elig,
        ...
        # Somatic surrogate for the soma path (beta_s); the plateau surrogate for
        # the dendrite. The dendritic-path somatic factor is chosen just below.
        ...
        if config.dend_surrogate_roof:
            # Flat roof over [v_th-gamma, v_th], evaluated on the pre-reset voltage
            # against the FIXED band (not the current-h dynamic threshold), so
            # credit spans the whole band a plateau could flip. gamma is folded
            # into the roof's width, so it is dropped as a separate factor.
            sp_hidden_d = surrogate_roof(
                h_v_pre, config.v_th - config.gamma, config.v_th, config.beta_s_dend,
            )
            eta_d = delta * sp_hidden_d * hp_hidden                   # (N,)
        else:
            # Single bump at the current effective threshold (original behavior).
            sp_hidden_d = surrogate_sigma(soma_dist, config.beta_s_dend)
            eta_d = delta * sp_hidden_d * config.gamma * hp_hidden    # (N,)
        grad_dend = grad_dend + eta_d[:, None] * dmu_atp_new

        return (h_carry, grad_soma, grad_dend), None

    grad_zeros = jnp.zeros((n_hidden, n_inputs))
    scan_inputs2 = (dend_inputs, soma_inputs, x_input, time_indices, rho)
    (_, grad_soma, grad_dend), _ = lax.scan(
        step2, (_hidden_zeros(), grad_zeros, grad_zeros), scan_inputs2,
    )
...
#
#  For _forward_backward:
#    x_input, target_smoothed, rng_key → batched (axis 0)
#    weights / alphas / config / loss constants / dropout_rate → shared (None)
#  Zero carries and grad accumulators are built inside the function, so they
#  need no batch axes here.
# ══════════════════════════════════════════════════════════════════════
...
class Network:
    ...
    # ── Helpers ──
    #  Zero carries and gradient accumulators are built inside
    #  _forward_backward from the weight shapes, so there is nothing to
    #  pre-allocate here.
    ...
    def train_step(self, x_input, target, lr=1e-3):
        ...
        """
        _, loss, pred, g_r, g_s, g_d = _fb_single(
    ...
    def batch_train_step(self, x_batch, targets, lr=1e-3):
        ...
        _, losses, preds, g_r, g_s, g_d = _fb_batch(
