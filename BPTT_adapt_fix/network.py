...
def _forward_backward(
    ...
    grad_a_init, grad_b_init, sum_Er_init,
    ...
    Returns: mean_voltage (J,), loss, prediction,
             ...
             grad_a_list, grad_b_list (each per-neuron N_l).
    """
    w_dend = weights["dend"]
    w_soma = weights["soma"]
    w_readout = weights["readout"]
    a_list = weights["a_adapt"]
    b_list = weights["b_adapt"]
    L = len(w_dend)
    ...
    def _layer_forward(h_state, dend_in_l, soma_in_l, presyn, t, tp_l, a_l, b_l):
        ...
        h_carry, o_l, v_pre_l, h_l, h_prev_l, mu_at_tp_l = TwoCompNeuron.forward_step(
            ...
            alpha_s, alpha_d, tp_l, config, alpha_w, a_l, b_l,
        )
        sp_l = surrogate_sigma(v_pre_l + config.gamma * h_l - config.v_th, config.beta_s)
        hp_l = surrogate_sigma(mu_at_tp_l - config.mu_th, config.beta_d)

        (mu_c, v_c, h_c, tp_c, matp_c, E_soma_c,
         dmu_c, dmu_atp_c, w_c, elig_a_prev, elig_b_prev) = h_carry
        ...
        # Adaptation-parameter eligibility: same alpha_w recursion as w. The
        # incoming elig_*_prev (= d w[t-1]/d param) is what pairs with Lv[t] in the
        # gradient; the advanced trace is stored for the next step. The a-term now
        # couples to the POST-reset voltage v_post (= v_pre·(1−o)), matching the
        # w recursion in TwoCompNeuron.forward_step.
        v_post_l = v_pre_l * (1 - o_l.astype(v_pre_l.dtype))
        elig_a_new = alpha_w * elig_a_prev + (1 - alpha_w) * v_post_l
        elig_b_new = alpha_w * elig_b_prev + o_l.astype(v_pre_l.dtype)
        new_state = (mu_c, v_c, h_c, tp_c, matp_c, E_soma_new, dmu_new, dmu_atp_new,
                     w_c, elig_a_new, elig_b_new)
        return (new_state, o_l, sp_l, hp_l, E_soma_new, dmu_atp_new,
                elig_a_prev, elig_b_prev)
    ...
    def step2(carry, inputs):
        h_states, grad_soma, grad_dend, grad_a, grad_b = carry
        dend_in0_t, soma_in0_t, x_t, t, drop_keys_t, rho_t = inputs
        x_t = x_t.astype(jnp.float64)

        new_h_states = list(h_states)
        sps, hps, Es, Phat, masks, eas, ebs = [], [], [], [], [], [], []
        o_prev = None
        for l in range(L):
            if l == 0:
                dend_in_l, soma_in_l, presyn = dend_in0_t, soma_in0_t, x_t
                ...
                soma_in_l = o_prev @ w_soma[l].T
                presyn = o_prev
            (new_h_states[l], o_l, sp_l, hp_l, E_soma_new, dmu_atp_new,
             elig_a_prev, elig_b_prev) = _layer_forward(
                h_states[l], dend_in_l, soma_in_l, presyn, t, tp_list[l],
                a_list[l], b_list[l],
            )
            mask = random.bernoulli(
            ...
            sps.append(sp_l); hps.append(hp_l)
            Es.append(E_soma_new); Phat.append(dmu_atp_new); masks.append(mask)
            eas.append(elig_a_prev); ebs.append(elig_b_prev)
        ...
        new_grad_a = list(grad_a)
        new_grad_b = list(grad_b)
        Lv = None
        for l in reversed(range(L)):
            ...
            # therefore carry the somatic pseudo-derivative: branch off Lv
            # (= delta_o·s'_s), NOT bare delta_o.
            new_grad_dend[l] = new_grad_dend[l] + (
                (Lv * config.gamma * hps[l])[:, None] * Phat[l]
            )
            # Per-neuron adaptation grads (descent-direction, matching w += lr*g):
            #   g_a = -Σ_t Lv[t]·elig_a[t-1],  g_b = -Σ_t Lv[t]·elig_b[t-1].
            new_grad_a[l] = new_grad_a[l] - Lv * eas[l]
            new_grad_b[l] = new_grad_b[l] - Lv * ebs[l]
        return (new_h_states, new_grad_soma, new_grad_dend,
                new_grad_a, new_grad_b), None

    scan_inputs2 = (dend_in_0, soma_in_0, x_input, time_indices, dropout_keys, rho)
    (_, grad_soma_f, grad_dend_f, grad_a_f, grad_b_f), _ = lax.scan(
        step2,
        (h_carry_init, grad_soma_init, grad_dend_init, grad_a_init, grad_b_init),
        scan_inputs2,
    )
    grad_soma_f = [g / _SOMA_GRAD_SCALE for g in grad_soma_f]

    return (mean_voltage, loss, prediction, grad_readout,
            grad_soma_f, grad_dend_f, grad_a_f, grad_b_f)
...
def _predict_only(
    ...
    leaving the eligibility slots of each carry untouched at zero. No dropout.
    """
    w_dend = weights["dend"]
    w_soma = weights["soma"]
    w_readout = weights["readout"]
    ...
    def step(carry, inputs):
        ...
        for l in range(L):
            ...
            h_carry, o_l, *_ = TwoCompNeuron.forward_step(
                ...
                alpha_s, alpha_d, tp_list[l], config, alpha_w, a_list[l], b_list[l],
            )
...
def _apply_grads(weights_flat, grads_flat, lr_flat, clip_value, wd_flat):
    """SGD with per-tensor LR and decoupled weight decay over a flat list of tensors.

    Gradients are clipped first, then w ← w + lr·g − lr·λ·w. lr_flat/wd_flat are
    per-tensor (matching weights_flat), letting the adaptation params run a scaled LR
    and stay out of weight decay. For plain SGD the decay term is equivalent to an L2
    penalty (λ/2)·||w||² but stays outside the clip so it can't be capped.
    """
    new = []
    for w, g, lr, wd in zip(weights_flat, grads_flat, lr_flat, wd_flat):
        g = jnp.clip(g, -clip_value, clip_value)
        new.append(w + lr * g - lr * wd * w)
    return new


def _adam_apply(
    weights_flat, grads_flat, m_flat, v_flat,
    step, lr_flat, beta1, beta2, eps, clip_value, wd_flat,
):
    """AdamW-style update with per-tensor LR and decoupled weight decay.

    lr_flat/wd_flat are per-tensor (matching weights_flat). Scaling the *learning rate*
    per tensor is what actually gives the adaptation params a smaller effective step —
    scaling their gradient instead would cancel in Adam's m̂/√v̂. wd=0 on those slots
    excludes them from decay.
    """
    new_w, new_m, new_v = [], [], []
    for w, g, m, v, lr, wd in zip(weights_flat, grads_flat, m_flat, v_flat, lr_flat, wd_flat):
        g = jnp.clip(g, -clip_value, clip_value)
        m = beta1 * m + (1 - beta1) * g
        v = beta2 * v + (1 - beta2) * g ** 2
        m_hat = m / (1 - beta1 ** step)
        v_hat = v / (1 - beta2 ** step)
        w = w + lr * m_hat / (jnp.sqrt(v_hat) + eps) - lr * wd * w
        new_w.append(w)
        new_m.append(m)
        new_v.append(v)
...
_FB_AXES = (
    ...
    0, 0, 0, 0, 0, 0, 0,               # h/r inits, grad_soma, grad_dend, grad_a, grad_b, sum_Er
    0,                                  # target_smoothed
...
class Network:
    def __init__(
        ...
        # Multiplier on the per-neuron a_adapt/b_adapt grads (an effective smaller
        # LR for the adaptation params). 1.0 = same LR as the weights.
        self.adapt_lr_scale = adapt_lr_scale
        ...
        # a_adapt clip ceiling (= a at f_max_hz); identical across layers (global taus).
        self.a_max = self.hidden[0].a_max
    ...
    # ordered list [dend_0, soma_0, a_0, b_0, .., readout] for the optimizer. Both
    # are rebuilt from the layer objects each call so they always reflect the
    # current weights. The per-neuron a_adapt/b_adapt are trained too, so they sit
    # in both views (canonical flat order: [dend_l, soma_l, a_l, b_l] then readout).
    ...
    def _weights(self):
        return {
            ...
            "a_adapt": [h.a_adapt for h in self.hidden],
            "b_adapt": [h.b_adapt for h in self.hidden],
        }
    ...
    def _weights_flat(self):
        ...
        for h in self.hidden:
            ...
            flat.append(h.a_adapt)
            flat.append(h.b_adapt)
    ...
    def _set_weights_flat(self, flat):
        ...
        for h in self.hidden:
            h.w_dend = flat[i]; i += 1
            h.w_soma = flat[i]; i += 1
            h.a_adapt = flat[i]; i += 1
            h.b_adapt = flat[i]; i += 1
        self.readout.w = flat[i]

    def _grads_flat(self, g_dend_list, g_soma_list, g_readout, g_a_list, g_b_list):
        flat = []
        for l in range(self.n_layers):
            flat.append(g_dend_list[l])
            flat.append(g_soma_list[l])
            flat.append(g_a_list[l])
            flat.append(g_b_list[l])
        flat.append(g_readout)
        return flat

    def _lr_flat(self, lr):
        """Per-tensor learning rates in _weights_flat order. The a/b slots get
        lr·adapt_lr_scale (an effective smaller LR); every other tensor gets lr.
        Scaling lr here — not the gradient — is what actually reaches Adam's step."""
        a = lr * self.adapt_lr_scale
        flat = []
        for _ in self.hidden:
            flat += [lr, lr, a, a]  # dend, soma, a_adapt, b_adapt
        flat.append(lr)             # readout
        return flat

    def _wd_flat(self):
        """Per-tensor weight decay in _weights_flat order. The a/b adaptation params
        are excluded (0.0) — an L2 pull toward LIF has no business shrinking them."""
        wd = self.weight_decay
        flat = []
        for _ in self.hidden:
            flat += [wd, wd, 0.0, 0.0]  # dend, soma, a_adapt, b_adapt
        flat.append(wd)                  # readout
        return flat
    ...
    def _h_carry(self, B=None):
        ...
        for l in range(self.n_layers):
            ...
            carries.append((
                ...
                jnp.zeros(s),
                jnp.zeros(s), jnp.zeros(s),  # elig_a, elig_b
            ))
        return carries
    ...
    def _acc_zeros(self, B=None):
        """Per-layer (N_l, N_{l-1}) weight-grad accumulators, per-neuron (N_l)
        a/b-grad accumulators, and the readout-elig sum vector."""
        dims = [self.n_inputs] + self.hidden_sizes
        grad_soma, grad_dend, grad_a, grad_b = [], [], [], []
        for l in range(self.n_layers):
            n, k = self.hidden_sizes[l], dims[l]
            shape = (B, n, k) if B else (n, k)
            nshape = (B, n) if B else (n,)
            grad_soma.append(jnp.zeros(shape))
            grad_dend.append(jnp.zeros(shape))
            grad_a.append(jnp.zeros(nshape))
            grad_b.append(jnp.zeros(nshape))
        er_shape = (B, self.hidden_sizes[-1]) if B else (self.hidden_sizes[-1],)
        sum_Er = jnp.zeros(er_shape)
        return grad_soma, grad_dend, grad_a, grad_b, sum_Er
    ...
    def _update_weights(self, g_dend_list, g_soma_list, g_readout,
                        g_a_list, g_b_list, lr, clip_value):
        # adapt_lr_scale is applied as a per-tensor LR multiplier (see _lr_flat), NOT
        # as a gradient scale — a gradient scale would cancel in Adam's m̂/√v̂.
        grads_flat = self._grads_flat(
            g_dend_list, g_soma_list, g_readout, g_a_list, g_b_list,
        )
        lr_flat = self._lr_flat(lr)
        wd_flat = self._wd_flat()
        if self.optimizer == "adam":
            self.adam_step = self.adam_step + 1
            new_w, self.m, self.v = _adam(
                self._weights_flat(), grads_flat, self.m, self.v,
                self.adam_step, lr_flat, self.beta1, self.beta2, self.adam_eps,
                clip_value, wd_flat,
            )
        else:
            new_w = _apply(
                self._weights_flat(), grads_flat, lr_flat, clip_value, wd_flat,
            )
        self._set_weights_flat(new_w)
        # Projected-gradient constraint: keep a_adapt in [0, a_max] (the resonant band)
        # and b_adapt in [0, 2]. Applied after the optimizer step (and after any weight
        # decay), so a/b can never leave the box even if a step pushes them out.
        for h in self.hidden:
            h.a_adapt = jnp.clip(h.a_adapt, 0.0, self.a_max)
            h.b_adapt = jnp.clip(h.b_adapt, 0.0, 2.0)

    def _next_key(self):
        self.rng_key, subkey = random.split(self.rng_key)
        return subkey

    def _grad_norms(self, g_dend_list, g_soma_list, g_readout, g_a_list, g_b_list):
        # a{l}/b{l} are the RAW adaptation grads (before adapt_lr_scale, which is
        # applied later in _update_weights). soma{l} is already post /_SOMA_GRAD_SCALE.
        ...
        for l in range(self.n_layers):
            ...
            norms[f"a{l}"] = float(jnp.linalg.norm(g_a_list[l]))
            norms[f"b{l}"] = float(jnp.linalg.norm(g_b_list[l]))
        return norms
    ...
    def train_step(self, x_input, target, lr=1e-3, clip_value=1.0):
        T = x_input.shape[0]
        gs0, gd0, ga0, gb0, Er0 = self._acc_zeros()

        (mean_v, loss, pred, g_r, g_s_list, g_d_list,
         g_a_list, g_b_list) = _fb_single(
            x_input, self._weights(), *self._params(),
            self._h_carry(), self._r_carry(), gs0, gd0, ga0, gb0, Er0,
            self._smooth_targets(target),
            self.config.loss_temperature, self.config.loss_count_bias,
            self._next_key(), self.dropout_rate,
        )

        gnorms = self._grad_norms(g_d_list, g_s_list, g_r, g_a_list, g_b_list)
        self._update_weights(g_d_list, g_s_list, g_r, g_a_list, g_b_list, lr, clip_value)
    ...
    def batch_train_step(self, x_batch, targets, lr=1e-3, clip_value=1.0):
        ...
        gs0, gd0, ga0, gb0, Er0 = self._acc_zeros(B)

        (mean_v, losses, preds, g_r, g_s_list, g_d_list,
         g_a_list, g_b_list) = _fb_batch(
            x_batch, self._weights(), *self._params(),
            self._h_carry(B), self._r_carry(B), gs0, gd0, ga0, gb0, Er0,
        ...
        g_a_avg = [jnp.mean(g, axis=0) for g in g_a_list]
        g_b_avg = [jnp.mean(g, axis=0) for g in g_b_list]

        gnorms = self._grad_norms(g_d_avg, g_s_avg, g_r_avg, g_a_avg, g_b_avg)
        self._update_weights(g_d_avg, g_s_avg, g_r_avg, g_a_avg, g_b_avg, lr, clip_value)
    ...
    def adapt_stats(self):
        """Per-layer diagnostics of the trained adaptation params: (mean, std) of
        a_adapt/b_adapt, the resonant frequency f (Hz) each a implies, and the fraction
        of neurons actually resonating (a > a_0). If f collapses toward 0 or the
        resonating fraction toward 0, the population is voting for plain SFA."""
        stats = {}
        dt = self.config.dt
        for l, h in enumerate(self.hidden):
            a_s, a_w = h.alpha_s, h.alpha_w
            a_0 = (jnp.sqrt(a_s) - jnp.sqrt(a_w)) ** 2 / (1 - a_w)  # oscillation onset
            cosphi = jnp.clip(
                (a_s + a_w - (1 - a_w) * h.a_adapt) / (2 * jnp.sqrt(a_s * a_w)), -1.0, 1.0,
            )
            f = jnp.arccos(cosphi) / (2 * jnp.pi * dt / 1000.0)  # 0 where a <= a_0
            stats[f"a{l}"] = (float(jnp.mean(h.a_adapt)), float(jnp.std(h.a_adapt)))
            stats[f"b{l}"] = (float(jnp.mean(h.b_adapt)), float(jnp.std(h.b_adapt)))
            stats[f"f{l}"] = (float(jnp.mean(f)), float(jnp.std(f)))
            stats[f"res{l}"] = float(jnp.mean(h.a_adapt > a_0))
        return stats
    ...
    def save(self, path, extra=None):
        meta = {
            ...
            "build": {
                ...
                "adapt_lr_scale": float(self.adapt_lr_scale),
            },
        ...
        for l, h in enumerate(self.hidden):
            ...
            arrays[f"a_adapt_{l}"] = np.asarray(h.a_adapt)
            arrays[f"b_adapt_{l}"] = np.asarray(h.b_adapt)
    ...
    def load(cls, path):
        ...
        net = cls(
            ...
            adapt_lr_scale=build.get("adapt_lr_scale", 1.0),
        )
        ...
        for l, h in enumerate(net.hidden):
            ...
            # Per-neuron adaptation params (fall back to the config-filled vectors
            # from __init__ for pre-adaptation checkpoints that lack them).
            if f"a_adapt_{l}" in data.files:
                h.a_adapt = arr(f"a_adapt_{l}")
                h.b_adapt = arr(f"b_adapt_{l}")
