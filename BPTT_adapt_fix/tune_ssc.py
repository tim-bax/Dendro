#!/usr/bin/env python3
"""Optuna hyperparameter search for BPTT_adapt_fix (trainable adaptation) on SSC.

Same eleven-knob search as BPTT_fix/tune_ssc.py, on the held-out VALIDATION split:
...
BPTT_adapt_fix trains per-neuron a_adapt/b_adapt, so adaptation is NOT tuned here;
the resonance controls (f_max_hz, adapt_init, adapt_lr_scale) and warm-start
a_adapt/b_adapt are FROZEN CLI knobs. tau_soma defaults to the resonant 30 ms.

SSC ships a dedicated ``valid`` split, so selection uses it directly and ``test``
is never touched. Retrain the winner with run_scc.py (reports test).

    python BPTT_adapt_fix/tune_ssc.py --precision 32 --n_trials 40 --hidden 128 128

Protocol note: TPE sampler + a light MedianPruner.
"""
import argparse
import json
import os
...
# ── Search space for the eleven tuned knobs (identical to BPTT_fix) ───────────
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Optuna tuning for BPTT_adapt_fix on SSC (val-driven, light pruning)."
    )
    # ── Search control ──
    ...
    p.add_argument("--storage", type=str, default="",
                   help="Optuna storage URL (e.g. sqlite:///bptt_adapt_fix_ssc.db). "
                        "Empty = in-memory.")
    p.add_argument("--study_name", type=str, default="bptt_adapt_fix_ssc_128")
    ...
    # ── Frozen biophysics / resonance recipe (fixed) ──
    p.add_argument("--tau_soma", type=float, default=30.0,
                   help="Soma time constant (fixed; resonant regime default 30 ms).")
    ...
    p.add_argument("--f_max_hz", type=float, default=20.0,
                   help="Resonant-band ceiling for per-neuron a_adapt init (fixed).")
    p.add_argument("--adapt_init", choices=["hetero", "warm"], default="hetero",
                   help="Per-neuron adaptation init (fixed; 'hetero' = frequency diversity).")
    p.add_argument("--a_adapt", type=float, default=0.0,
                   help="Warm-start a_adapt (used only with --adapt_init warm).")
    p.add_argument("--b_adapt", type=float, default=0.0,
                   help="Warm-start b_adapt (used only with --adapt_init warm).")
    p.add_argument("--adapt_lr_scale", type=float, default=1.0,
                   help="LR multiplier for the trained a_adapt/b_adapt params (fixed).")
    ...
    # NOTE: gamma, beta_s, beta_d, tau_plat_min/max, channel_shift_range are TUNED.
    # ── Training (fixed) ──
...
def build_config(args, *, loss_temperature, loss_count_bias, loss_label_smoothing,
    ...
    return NeuronConfig(
        ...
        f_max_hz=args.f_max_hz, adapt_init=args.adapt_init,
...
def main():
    ...
    print(
        ...
        f"tau_w={args.tau_w} f_max_hz={args.f_max_hz} adapt_init={args.adapt_init} "
        f"adapt_lr_scale={args.adapt_lr_scale} a_adapt={args.a_adapt} "
        f"b_adapt={args.b_adapt} mu_th={args.mu_th} v_th={args.v_th} "
        f"dropout={args.dropout} lr_patience={args.lr_patience}",
        flush=True,
    )
    ...
    def objective(trial):
        ...
        net = Network(
            ...
            weight_decay=args.weight_decay, adapt_lr_scale=args.adapt_lr_scale,
        )
    ...
    print(
        ...
        f"  python BPTT_adapt_fix/run_scc.py --hidden {hidden_str.replace('-', ' ')} "
        f"--n_outputs {args.n_outputs} --optimizer {args.optimizer} "
        f"--precision {args.precision} --batch_size {args.batch_size} "
        f"--epochs {args.epochs} --eval_split test "
