#!/usr/bin/env python3
"""Optuna hyperparameter search for BPTT_adapt_fix (trainable adaptation) on SHD.

SHD counterpart of BPTT_adapt_fix/tune_ssc.py: the SAME eleven-knob search on the
...
Adaptation is trained per-neuron, so it is NOT tuned; the resonance controls
(f_max_hz, adapt_init, adapt_lr_scale) and warm-start a_adapt/b_adapt are FROZEN
CLI knobs. tau_soma defaults to the resonant 30 ms.

SHD ships no validation split, so some train SPEAKERS are held out as val; the SHD
*test* set is never touched. Retrain the winner on ALL train speakers with
run_shd.py (no --val_speakers).

    python BPTT_adapt_fix/tune_shd.py --precision 32 --n_trials 40 --hidden 128 128
"""
import argparse
import json
import os
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Optuna tuning for BPTT_adapt_fix on SHD (val-driven, light pruning)."
    )
    # ── Search control ──
    ...
    p.add_argument("--storage", type=str, default="",
                   help="Optuna storage URL (e.g. sqlite:///bptt_adapt_fix_shd.db). "
                        "Empty = in-memory.")
    p.add_argument("--study_name", type=str, default="bptt_adapt_fix_shd_128")
...
def main():
    ...
    print(
        ...
        f"  python BPTT_adapt_fix/run_shd.py --hidden {hidden_str.replace('-', ' ')} "
