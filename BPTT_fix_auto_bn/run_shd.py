...
def parse_args():
    ...
    # sparch-style BatchNorm1d on the per-layer pre-synaptic current.
    p.add_argument("--normalization", choices=["batchnorm", "none"], default="batchnorm")
    p.add_argument("--bn_targets", choices=["both", "soma", "dend"], default="both",
                   help="which projection(s) get a BatchNorm when normalization=batchnorm")
...
def main():
    ...
    norm_str = (f"  batchnorm({args.bn_targets})"
                if args.normalization == "batchnorm" else "  normalization=none")
    hidden_str = " -> ".join(f"{n} (2-comp)" for n in args.hidden)
    arch_str = f"{n_inputs} -> {hidden_str} -> {args.n_outputs} (LI readout)"
    print(
        f"Network: {arch_str}  full-gradient autodiff BPTT{norm_str}  "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if gnorm_count > 0:
            # Sort by trailing layer index then name, so keys like dend0/soma0
            # and BN keys like bn_soma_g0 group by layer; readout goes last.
            def _gnorm_key(k):
                m = re.search(r"(\d+)$", k)
                idx = int(m.group(1)) if m else 999
                prefix = k[: m.start()] if m else k
                return (idx, prefix)
