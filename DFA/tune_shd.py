#!/usr/bin/env python3
"""Optuna hyperparameter search for the DFA network on SHD.

Rewritten to mirror the BPTT_fix search: optimizes the held-out-SPEAKER
VALIDATION accuracy over the SAME eleven shared knobs PLUS the DFA-specific
``feedback_scale`` (twelve total):
...
Everything else (extra biophysics, dropout, weight_decay, batch_size, LR-schedule
shape) is FROZEN and passed on the CLI. Uses the shared ``trainer.train_model``
(selection = smoothed val acc; Optuna pruning is applied inside the loop).

    python DFA/tune_shd.py --precision 32 --n_trials 40 --hidden 128 128

Protocol: tune with a speaker-holdout val set, then retrain the winner on ALL
train speakers (run_shd.py without --val_speakers) before reporting test.
"""
import argparse
import json
import os
...
from trainer import split_by_speakers, train_model
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Optuna tuning for DFA on SHD (val-driven, light pruning)."
    )
    # ── Search control ──
    ...
    p.add_argument("--early_stop_patience", type=int, default=0,
                   help="Per-trial early stopping on the smoothed val metric "
                        "(0 = disabled; rely on pruning to truncate bad trials).")
    p.add_argument("--progress_every", type=int, default=5,
                   help="Print a compact per-trial heartbeat every N epochs.")
    p.add_argument("--storage", type=str, default="",
                   help="Optuna storage URL (e.g. sqlite:///dfa_shd.db) for a "
                        "resumable study. Empty = in-memory.")
    p.add_argument("--study_name", type=str, default="dfa_shd_128")
    ...
    # NOTE: gamma, beta_s, beta_d, tau_plat_min/max, channel_shift_range and
    # feedback_scale are TUNED (see the objective).
    # ── Training (fixed) ──
...
def main():
    ...
    print(
        ...
        f"val_speakers={sorted(args.val_speakers)} (val N={len(val_data)}, "
        f"train N={len(train_data)})  trials={args.n_trials}  "
        f"epochs/trial={args.epochs}  bs={args.batch_size}  pruner={args.pruner}",
        flush=True,
    )
    print(
    ...
    print(
        ...
        f"  python DFA/run_shd.py --hidden {hidden_str.replace('-', ' ')} "
