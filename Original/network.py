...
# dendritic gradient (A_soma runs ~8x larger). Empirical, not derived — see the
# comment in _loss_and_grads. Results were obtained at 8.0; changing it reweights
# somatic vs dendritic learning.
...
def _loss_and_grads(
    ...
    #
    # The somatic gradient carries an EXTRA 1/8 (_SOMA_GRAD_SCALE) that balances it
    # against the dendritic one: A_soma comes out roughly 8x larger than A_dend, so
    # without this the somatic pathway dominates learning and the dendritic weights
    # barely move. The factor is empirical, not derived — it is a tuned constant.
    grad_readout = (global_error[:, None] * A_readout) / T
    grad_soma = jnp.einsum("j,jik->ik", global_error, A_soma) / (T * _SOMA_GRAD_SCALE)
